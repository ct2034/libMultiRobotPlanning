...
class Animation:
  def __init__(self, map, schedule):
    ...
    self.patches.append(Rectangle((xmin, ymin), xmax - xmin,
                        ymax - ymin, facecolor='none', edgecolor='red'))
    for o in map["map"]["obstacles"]:
      x, y = o[0], o[1]
      self.patches.append(Rectangle((x - 0.5, y - 0.5), 1,
                          1, facecolor='red', edgecolor='red'))
    ...
    for d, i in zip(map["agents"], range(0, len(map["agents"]))):
      ...
      for goal in goals:
        self.patches.append(Rectangle((goal[0] - 0.25, goal[1] - 0.25), 0.5,
                            0.5, facecolor=Colors[i % len(Colors)], edgecolor='black', alpha=0.5))

    for d, i in zip(map["agents"], range(0, len(map["agents"]))):
      name = d["name"]
      self.agents[name] = Circle((d["start"][0], d["start"][1]),
                                 0.3, facecolor=Colors[i % len(Colors)], edgecolor='black')
...
if __name__ == "__main__":
    ...
    with open(args.map) as map_file:
        map = yaml.load(map_file, Loader=yaml.SafeLoader)
